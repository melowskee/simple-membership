    ...
    <div class="overflow-hidden overflow-x-auto min-w-full align-middle sm:rounded-md">
        <div class="flex place-content-end mb-4">
            <div class="px-4 py-2 text-white bg-indigo-600 hover:bg-indigo-700 cursor-pointer">
                <router-link :to="{ name: 'branches.create' }" class="text-sm font-medium">Create Branch</router-link>
            </div>
        </div>
        ...
        <table class="min-w-full border divide-y divide-gray-200">
            ...
            <template v-for="item in branches" :key="item.id">
                <tr class="bg-white">
                    ...
                    <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                        <router-link :to="{ name: 'branches.edit', params: { id: item.id } }"
...
export default {
    setup() {
        const { branches, getBranches, destroyBranch } = useBranches()
